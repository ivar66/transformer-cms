                    ...
                    <form role="form" name="item_form" id="item_form" method="post">
                        ...
                        <div class="box-header">
                            <div class="row">
                                <div class="col-xs-12">
                                    <div class="btn-group">
                                        <a href="{{ route('admin.category.create') }}" class="btn btn-default btn-sm"
                                           data-toggle="tooltip" title="添加分类"><i class="fa fa-plus"></i></a>
                                        <button type="button" class="btn btn-default btn-sm" data-toggle="tooltip"
                                                title="删除选中项"
                                                onclick="confirm_submit('item_form','{{  route('admin.category.destroy') }}','删除选中分类会同时删除其子分类，确认继续操作？')">
                                ...
                                <table class="table table-striped">
                                    ...
                                    @foreach($categories as $category)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="label @if($category->status==2) label-danger  @else label-success @endif">{{ trans_common_status($category->status) }}</span>
                                            </td>
                                            <td>
                                                <div class="btn-group-xs">
                                                    <a class="btn btn-primary"
                                                       href="{{ route('admin.category.edit',['id'=>$category->id]) }}"
                                                       data-toggle="tooltip" title="编辑分类信息"><i class="fa fa-edit"
                                                                                               aria-hidden="true"></i>
                                                        编辑</a>
                                                </div>
                                            </td>
