...
<body class="skin-blue sidebar-mini @if( isset($sidebar_collapse) && $sidebar_collapse) sidebar-collapse @endif">
<div class="wrapper">
                ...
                <ul class="nav navbar-nav">
                    ...
                    <li class="dropdown notifications-menu">
                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">
                            ...
                            @if(isset($notVerifiedData) && array_sum($notVerifiedData)>0)
                                <span class="label label-warning">@if(array_sum($notVerifiedData)>99)
                                        99+@else{{ array_sum($notVerifiedData) }}@endif</span>
                            @endif
                        </a>
                        <ul class="dropdown-menu">
                            <li class="header">共有 @if(isset($notVerifiedData)) {{ array_sum($notVerifiedData) }} @endif 个待处理事项</li>
                            <li>
                                        ...
                                        @if( $notVerifiedData['questions'] > 0 )
                                            <li>
                                                <a href="{{ route('admin.question.index') }}?status=0">
                                                    <i class="fa fa-question-circle text-yellow"></i> {{ $notVerifiedData['questions'] }}
                                                    个问题需要审核
                                                </a>
                                            </li>
                                        @endif
                                        @if( $notVerifiedData['answers'] > 0  )
                                            <li>
                                                <a href="{{ route('admin.answer.index') }}?status=0">
                                                    <i class="fa fa-comment-o text-yellow"></i> {{ $notVerifiedData['answers'] }}
                                                    个回答需要审核
                                                </a>
                                            </li>
                                        @endif
                                        @if( $notVerifiedData['articles'] > 0  )
                                            <li>
                                                <a href="{{ route('admin.article.index') }}?status=0">
                                                    <i class="fa fa-book text-yellow"></i> {{ $notVerifiedData['articles'] }}
                                                    篇文章需要审核
                                                </a>
                                            </li>
                                        @endif
                                        @if( $notVerifiedData['comments'] > 0  )
                                            <li>
                                                <a href="{{ route('admin.comment.index') }}?status=0">
                                                    <i class="fa fa-comments-o text-yellow"></i> {{ $notVerifiedData['comments'] }}
                                                    个评论需要审核
                                                </a>
                                            </li>
                                        @endif
                    ...
                    <li class="dropdown user user-menu">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            <li class="user-header">
                                <img src="{{ get_user_avatar(Auth()->user()->id,'middle') }}" class="img-circle"
                                     alt="User Image"/>
                                <p>
                                    {{ Auth()->user()->name }}
