...
@section('seo_title')话题 @if($topics->currentPage()>1)- 第{{ $topics->currentPage() }}页 @endif - @endsection
@section('content')
        ...
        @foreach($topics as $topic)
            <section class="topic-list-item col-md-3">
                <div class="widget-topic">
                    <h2 class="h4">
                        <a href="{{ route('web.topic.detail',['id'=>$topic->id]) }}" @if($topic->logo) class="tag-logo" style="background-image: url({{ route('website.image.show',['image_name'=>$topic->logo]) }});" @endif>{{ $topic->tag_name }}</a>
                    </h2>
                    <p>
